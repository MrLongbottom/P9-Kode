...
if __name__ == '__main__':
    ...
    query_key = list(queries.keys())[2]
    query_content = list(queries.values())[2].split(' ')
    dt = sp.load_npz("Generated Files/topic_doc_matrix.npz")
    tw = sp.load_npz("Generated Files/topic_word_matrix.npz")
    res1, p_vec1 = query_handling.lda_evaluate_query(query_key, query_content, tell=False, dt_matrix=dt, tw_matrix=tw)
    res2, p_vec2 = language_model.lm_evaluate_query(query_key, query_content, tell=False)
