...
def log_perplexity(lda: LdaModel, dictionary: Dictionary, doc2word):
    corpus = [dictionary.doc2bow(text) for text in doc2word.values()]
...
if __name__ == '__main__':
    ...
    dictionary = Dictionary(list(doc2word.values()))
    K = math.floor(math.sqrt(cv.shape[0]) / 2)
    # lda = run_lda('LDA/model/document_model',
    #         cv,
    #         words,
    #         corpus,
    #         Dictionary(list(corpus.values())),
    #         "Generated Files/",
    #         (K,None,None),
    #         tw_threshold=0.001,
    #         dt_threshold=0.025)
    ...
    corpus = [dictionary.doc2bow(text) for text in doc2word.values()]
    print(log_perplexity(lda, dictionary, doc2word))
